...
class AppServiceProvider extends ServiceProvider
{
     ...
     * пропуск для неё снят. Закрытый материал администратор читает и правит
     * (см. CourseAccess), но круг допущенных остаётся за автором, и такого
     * отказа в обход политики не выразить. Права как таковые должность
     * по-прежнему даёт: «редактировать курсы» спрашивают без курса в
     * аргументах, и такая проверка проходит.
     */
     ...
     *
     * Оба умеют быть закрытыми, и у обоих кругом допущенных распоряжается
     * автор, а не должность, — поэтому для них пропуск снимается и решение
     * остаётся за политикой. См. CourseAccess и RegulationAccess.
     *
