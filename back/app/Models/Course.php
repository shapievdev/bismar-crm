...
class Course extends Model implements PartOfCourse
{
    ...
    /**
     * Группы, которым открыт приватный курс (2026-09-12).
     *
     * Складывается с поимённым списком, а не заменяет его: «весь отдел продаж
     * плюс Иванов из соседнего» — обычная просьба. Состав группы читается
     * живьём, на каждом обращении, а не замораживается при допуске: ради этого
     * группу и называют вместо двадцати фамилий. Само правило — в CourseAccess.
     *
     * @return BelongsToMany<Group, $this>
     */
    public function memberGroups(): BelongsToMany
    {
        return $this->belongsToMany(Group::class, 'course_member_groups')
            ->withPivot('granted_by_id')
            ->withTimestamps();
    }
