...
return [
    ...
    */
    ...
    /*
     * Ниже этой близости кусок текста не про этот вопрос.
     *
     * Нужен смысловому поиску по расшифровкам, и только ему: у поиска по словам
     * порог свой, относительный, а у смыслового его не было бы вовсе. Без него
     * ближайшие восемь кусков находятся всегда — в том числе на вопрос, к
     * которому в базе нет ничего, — и честный ответ «в базе об этом ничего нет»
     * становится недостижим, а с ним и совет посмотреть близкое.
     *
     * Своя шкала, не та, что у ai.answers_floor: там вопрос сравнивают с
     * вопросом, здесь — с куском текста, и вектор длинного куска систематически
     * дальше. Переносить порог с одной шкалы на другую нельзя.
     *
     * Замер на живой базе (text-embedding-3-small, 2026-09-08). Вопросы, ответ
     * на которые в базе есть: медиана 0.61, четверть худших ниже 0.52, худший
     * 0.20. Вопросы не про эту компанию вовсе («как поменять картридж»,
     * «столица Австралии»): медиана 0.29, лучший 0.43. Распределения
     * перекрываются, и одним числом их не разделить.
     *
     * Отсюда 0.35 — сторона своих: пропускает 98% вопросов, на которые ответ
     * есть, и отсекает 70% посторонних. Порог выше 0.40 отсекал бы уже каждый
     * двенадцатый настоящий вопрос, а это дороже: посторонний кусок модель
     * узнаёт и сама — ей велено в таком случае сказать, что материала нет, — а
     * выброшенный ответ вернуть неоткуда.
     *
     * На другой модели эмбеддингов шкала другая: перемеряйте, а не переносите.
     */
    'passages_floor' => (float) env('AI_PASSAGES_FLOOR', 0.35),
    ...
    */
    ...
    /*
    |--------------------------------------------------------------------------
    | Порог схожести письменного ответа
    |--------------------------------------------------------------------------
    |
    | Насколько ответ сотрудника своими словами должен быть похож на эталон
    | автора, чтобы вопрос был зачтён (решение пользователя 2026-09-02).
    |
    | Два числа, потому что мерки две и шкалы у них разные — см.
    | App\Support\Lms\AnswerSimilarity:
    |
    |  - по смыслу (эмбеддинги): 0.60. Число замерено на настоящих ответах
    |    (text-embedding-3-small, 2026-09-02): верный ответ своими словами даёт
    |    0.69-0.70, ответ по теме, но неверный — 0.43, ответ не по теме — 0.24,
    |    отписка «не знаю» — 0.17. Порог стоит посередине разрыва между верным
    |    пересказом и неверным ответом: так у него запас в обе стороны.
    |
    |    Не путайте с ai.verbatim_threshold (0.75): тот сравнивает вопрос с
    |    вопросом, а здесь ответ с ответом, и шкала там выше. На другой модели
    |    эмбеддингов перемеряйте, а не переносите.
    |  - по словам (доля общих, запасной путь без эмбеддингов): 0.25. Замер на
    |    тех же ответах: верный пересказ даёт 0.32, неверный по теме — 0.16, не
    |    по теме — 0.00. Пересказ теряет половину слов, поэтому шкала здесь
    |    вдвое ниже смысловой, и переносить порог с неё нельзя.
    |
    */

    'answer_similarity_floor' => (float) env('AI_ANSWER_SIMILARITY_FLOOR', 0.60),

    'answer_similarity_floor_by_words' => (float) env('AI_ANSWER_SIMILARITY_FLOOR_BY_WORDS', 0.25),
