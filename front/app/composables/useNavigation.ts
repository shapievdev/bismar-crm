...
export function useNavigation() {
  ...
  const items = computed<NavItem[]>(() => [
    ...
    {
      /*
       * Раздел открыт двоим: тому, кому доверено обучение, и тому, кому
       * доверены деньги. Открывается он обучением — там курсы и документы,
       * которые касаются всех; за выручкой ходит меньше людей и реже. Кому
       * обучение не доверено, тот попадает сразу на продажи: пустая страница
       * вместо раздела хуже, чем другая его вкладка.
       */
      to: can('enrollments.manage') ? '/analytics' : '/analytics/sales',
      label: 'Аналитика',
      icon: 'analytics',
