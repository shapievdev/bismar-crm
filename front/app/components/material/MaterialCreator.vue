...
/**
 * Заведение документа или справочника — один экран на оба раздела.
 */
const props = defineProps<{ section: MaterialSection }>()

const copy = useMaterialSection(props.section)

useHead({ title: copy.createLabel })

const { createRegulation, fetchCategories } = useMaterialsApi(props.section)
const router = useRouter()

const { data: categoryData } = await useAsyncData(`lms.${props.section}.categories.new`, () => fetchCategories())
...
async function save() {
  ...
  try {
    const { data } = await createRegulation({
      ...
      // Ключевые слова пишут на экране правки, рядом со статьёй: здесь ещё
      // неизвестно, о чём документ будет.
      keywords: [],
    })

    await router.push(`/lms/${copy.section}/${data.slug}/edit`)
  }
  catch (caught) {
    if (caught instanceof ApiValidationError) {
      ...
      generalError.value = copy.createFailed
    }
  }
  finally {
  ...
  <section>
    ...
    <form class="card form" novalidate @submit.prevent="save">
      ...
      <div class="form__actions">
        ...
        <NuxtLink :to="`/lms/${copy.section}`" class="button-ghost">
          Отмена
        </NuxtLink>
      </div>
    </form>
